...
public class PathingMain extends PApplet
{
   ...
   */
   private boolean moving(Point pos, GridValues[][] grid, List<Point> path)
   {
      try {
         Thread.sleep(1);
      } catch (Exception e) {
      ...
      // Validates if cell is valid or not
      ...
      if (withinBounds(pos, grid) && grid[pos.y][pos.x] != GridValues.OBSTACLE && grid[pos.y][pos.x] != GridValues.SEARCHED) {
         ...
         if (grid[pos.y][pos.x] == GridValues.GOAL) {
            ...
            valid = moving(rightN, grid, path) ||  moving(leftN, grid, path) ||  moving(downN, grid, path) || moving(upN, grid, path);
         }
